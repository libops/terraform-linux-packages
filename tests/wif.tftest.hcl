...
run "requires_managed_approved_reusable_workflows" {
  command = plan

  assert {
    condition = (
      strcontains(local.provider_attribute_condition, "assertion.repository == 'libops/sitectl'") &&
      strcontains(local.provider_attribute_condition, "assertion.job_workflow_ref == 'libops/terraform-linux-packages/.github/workflows/reusable-goreleaser.yaml@refs/heads/main'") &&
      strcontains(local.provider_attribute_condition, "assertion.job_workflow_ref == 'libops/.github/.github/workflows/sitectl-plugin-goreleaser.yaml@refs/heads/main'") &&
    ...
    error_message = "WIF must require the caller repository and one of the managed direct/shared reusable workflow identities."
  }
}

run "rejects_sha_pinned_workflow_identity" {
  command = plan
