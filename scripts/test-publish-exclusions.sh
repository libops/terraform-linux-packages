  ...
  "storage rsync --recursive --checksums-only --cache-control=asset-contract "*)
    ...
    while IFS= read -r -d '' asset_file; do
      ...
      case "$relative_path" in
        sitectl_0.9.0_amd64.deb)
          grep -Fxq 'historical core deb' "$asset_file"
          printf 'ROLLBACK_BYTES %s\n' "$relative_path" >>"$log_file"
          ;;
        sitectl-0.9.0-1.x86_64.rpm)
          grep -Fxq 'historical core rpm' "$asset_file"
          printf 'ROLLBACK_BYTES %s\n' "$relative_path" >>"$log_file"
          ;;
      esac
...
grep -Fq "ROLLBACK_BYTES sitectl_0.9.0_amd64.deb" "$gcloud_log"
grep -Fq "ROLLBACK_BYTES sitectl-0.9.0-1.x86_64.rpm" "$gcloud_log"
